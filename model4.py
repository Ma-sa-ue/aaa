...
class DCGAN():
    def __init__(
        ...
        self.gen_W2 = tf.Variable(tf.random_normal([4,4,dim_W2, dim_W1], stddev=0.02), name='gen_W2')
        ...
        '''
        self.gen_W4 = tf.Variable(tf.random_normal([4,4,dim_W4, dim_W2], stddev=0.02), name='gen_W4')
        ...
        self.gen_W5 = tf.Variable(tf.random_normal([4,4,dim_W5, dim_W4], stddev=0.02), name='gen_W5')

        self.discrim_W1 = tf.Variable(tf.random_normal([4,4,dim_W5,dim_W4], stddev=0.02), name='discrim_W1')

        self.discrim_W2 = tf.Variable(tf.random_normal([4,4,dim_W4,dim_W2], stddev=0.02), name='discrim_W2')
        self.discrim_bn_g2 = tf.Variable( tf.random_normal([dim_W2], mean=1.0, stddev=0.02), name='discrim_bn_g2')
        self.discrim_bn_b2 = tf.Variable( tf.zeros([dim_W2]), name='discrim_bn_b2')
        '''
        ...
        '''
        self.discrim_W4 = tf.Variable(tf.random_normal([4,4,dim_W2,dim_W1], stddev=0.02), name='discrim_W4')
    ...
    def build_model(self):
        ...
        ###gen_cost = tf.reduce_mean((p_gen-1)*(p_gen-1))
        ###gen_cost = tf.reduce_mean(-p_gen*p_gen)
        ###hey = p_gen/(1-p_gen)
        ###gen_cost = -tf.reduce_mean(-p_gen)
        ###gen_cost = -tf.reduce_mean(hey*hey)
        ###gen_cost = tf.reduce_mean(p_gen*tf.log(p_gen+p_real))
        ##gen_cost = tf.reduce_mean(-0.5*tf.reduce_mean(p_gen*p_gen)-0.5*tf.reduce_mean(p_real*p_real)))
        ###gen_cost = tf.reduce_mean(p_gen*tf.log(p_gen)-p_gen+1)
        ####gen_cost = tf.reduce_mean(-p_gen)
        ###gen_cost = tf.reduce_mean(bce( p_gen, tf.ones_like(p_gen) ))
        ###hey = p_gen/(1-p_gen)
        ###gen_cost = tf.reduce_mean(-tf.log(1e-7+hey))
    ...
    def generate(self, Z):
        ...
        '''
        output_shape_l4 = [self.batch_size,16,16,self.dim_W4]
        h4 = tf.nn.conv2d_transpose(h2, self.gen_W4, output_shape=output_shape_l4, strides=[1,2,2,1])
        h4 = tf.nn.relu( batchnormalize(h4, g=self.gen_bn_g4, b=self.gen_bn_b4) )

        output_shape_l5 = [self.batch_size,32,32,self.dim_W5]
    ...
    def samples_generator(self, batch_size):
        ...
        '''
        output_shape_l4 = [batch_size,16,16,self.dim_W4]
        h4 = tf.nn.conv2d_transpose(h2, self.gen_W4, output_shape=output_shape_l4, strides=[1,2,2,1])
        h4 = tf.nn.relu( batchnormalize(h4) )

        output_shape_l5 = [batch_size,32,32,self.dim_W5]
        h5 = tf.nn.conv2d_transpose(h4, self.gen_W5, output_shape=output_shape_l5, strides=[1,2,2,1])
